...
object JsonSerdes {
  ...
  private def commandRequest[K: Encoder: Decoder, C: Encoder: Decoder] =
    productCodecs4[UUID, K, Long, C, CommandRequest[K, C]]("key",
                                                           "command",
                                                           "readSequence",
                                                           "commandId")(
      v =>
        (v.commandId().id,
         v.aggregateKey(),
         v.readSequence().getSeq,
         v.command()),
      (id, k, rs, c) =>
        new CommandRequest(CommandId.of(id), k, Sequence.position(rs), c)
    ).asSerde

  def commandSerdes[K: Encoder: Decoder, C: Encoder: Decoder]
    : CommandSerdes[K, C] =
  ...
  def actionSerdes[A: Encoder: Decoder]: ActionSerdes[A] = new SharedCodecs[A] with ActionSerdes[A] {
    import io.simplesource.saga.model.messages._

    private val req =
      productCodecs6[UUID, UUID, UUID, A, String, Boolean, ActionRequest[A]](
        "sagaId",
        "actionId",
        "commandId",
        "command",
  ...
  def sagaSerdes[A: Encoder: Decoder]: SagaSerdes[A] = new SharedCodecs[A] with SagaSerdes[A] {
    import io.simplesource.saga.model.messages._
    import io.simplesource.saga.model.saga._
    import ResultEncoders._

    private implicit val (acEnc, acDec) =
      productCodecs3[UUID, A, String, ActionCommand[A]]("commandId",
                                                        "command",
                                                        "actionType")(
    ...
    private implicit val (saEnc, saDec) = productCodecs6[UUID,
                                                 ActionCommand[A],
    ...
    private implicit val (sagaEnc, sagaDec) =
      productCodecs4[SagaId,
                     java.util.Map[ActionId, SagaAction[A]],
                     String,
                     Sequence,
                     Saga[A]](
        "sagaId",
        ...
        (sid, acts, st, seq) =>
          Saga.of[A](sid, acts, SagaStatus.valueOf(st), seq))
    ...
    private val sagaResponseSerde =
      productCodecs2[SagaId, Result[SagaError, Sequence], SagaResponse](
        "sagaId",
        "initialState")(
        x => (x.sagaId, x.result),
    ...
    private implicit val (ascEnc, ascDec) =
      productCodecs5[SagaId,
        ...
        x =>
          (x.sagaId,
           x.actionId,
           x.actionStatus.toString,
           x.actionErrors,
           x.undoCommand),
        (sid, aid, st, e, u) =>
          SagaStateTransition.SagaActionStateChanged
            .of(sid, aid, ActionStatus.valueOf(st), e, u)
      )

    private implicit val (sscEnc, sscDec) =
      productCodecs3[SagaId,
                     String,
                     java.util.List[SagaError],
        ...
        (sid, ss, es) =>
          SagaStateTransition.SagaStatusChanged.of(sid,
                                                   SagaStatus.valueOf(ss),
                                                   es))
      ...
      productCodecs4[
        ...
        (is, as, ss, tl) =>
          (is, as, ss, tl) match {
            ...
            case _ =>
              throw new Exception(
                "Error in SagaStateTransition deserialization")
        }
      ).asSerde
